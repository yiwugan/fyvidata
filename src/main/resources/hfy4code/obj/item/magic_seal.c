...
void create()
{
        ...
        set("value", 0);
        set("fast_grab",1);
		  if( clonep(this_object()) ) call_out("lostseal", 890,1);
} 
int unequip()
{
        set("equipped", 0);
...
int do_write(string arg)
{
        ...
        if( !tarobj ) {
           ...
           obj->set("long",
                "这是一张" + obj->name() + "，上面写着＂" + target + "＂几个血字！\n");
        }
        else {
           message_vision("$N把" + tarobj->name() + "写在$n上。\n", this_player(), obj);
           obj->set("long",
                "这是一张" + obj->name() + "，上面写着＂" + tarobj->name() + "＂几个血字！\n");
        }
...
//      this_player()->start_busy(1);
        ...
        destruct(sobj);
        return notify_fail("你将手中的符烧了，但是什么事情也没有发生,让你足实迷糊了一阵。。\n");
} 
...
int lostseal(object me)
{
         object seal;
       write("你画的符纸渐渐的变的模糊不清。\n");
                seal = new("/obj/item/paper_seal");
                seal->move(this_player());
