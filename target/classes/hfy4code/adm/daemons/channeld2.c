...
//#define MAX_CHANNEL_ONE_SECTION               4
...
static mapping channels = ([
    ...
    "fy"   :(["name"     :"风云",
              ...
              "flag"     : C_LIMITNEW,
              "intermud" : "61.144.192.43",
              "channel"  : "fy",
              "filter"   :(: $1["VERSION"]=="sumxin" :)  ]),
    "fywiz":(["name"     :"风云巫师",
              "msg_speak": HIY"【%s】%s：%s\n"NOR,
              "msg_emote": HIY"【%s】%s"NOR,
              "flag"     : C_WIZ,
...
int do_sys_channel(string verb, string arg)
{
        ...
        // Do the logging
        if( channel["flag"]&C_LOG ) log_file(LOG_FILE,sprintf("时间:%s 频道:%s 信息:%s\n",
                       ...
                       int cemote)
{
     ...
     /* if (interactive(me) && !wizardp(me))
        ...
        else //It's a public channel (verb is mapped in channels)
        {
                ...
                if(userp(me) && (flag&C_WIZ) && !wizardp(me) ) return 0; 
                ...
                if (userp(me)) 
                {
                   tuned_ch =  me->query("channels");
                        ...
                        log_file(LOG_FILE,sprintf("时间:%s 频道:%s 人物:%s(%s) IP:%s(%s) 信息:%s\n",
                        ctime(time()),channel["name"],me->query("name"),
                        me->query("id"),query_ip_name(me),query_ip_number(me),arg)); 
