...
string *banned_name = ({
        ...
        "苗族",  "满族","蒙古族","风云ＩＩＩ","风云Ⅲ",
        	"书剑", "百姓", "某些人", "管理",
	"某人", "巫师", "天神", "总管", "龙神", "仙人", "门客", "玩家",
	"尸体", "我", "你", "他", "她", "它", "大家", "自己", "自已",
	"中国", "中华", "皇帝", "鬼魂", "头颅", "杀人", "蒙面",
		"毛泽东", "邓小平", "江泽民", "朱容基", "钱其琛", "李瑞环", "胡耀邦",
	"金庸", "查良镛", "古龙", "黄易", "鲁迅", "郭沫若", "矛盾", "老舍",
		"丨", "丿", "丶", "匚", "刂", "冂", "亻", "勹", "亠", "冫", "冖",
	"讠", "卩", "阝", "廴", "凵", "厶", "艹", "廾", "扌", "囗", "彳",
	"彡", "犭", "饣", "夂", "忄", "丬", "氵", "宀", "辶", "彐", "屮",
	"纟", "巛", "灬", "礻", "肀", "疒", "衤",
		"屁", "鸡", "性", "淫", "嫖", "阴", "操", "奸", "妓", "睾丸", "恭喜发财",
	"赤佬", "瘪三", "傻逼", "蠢蛋", "阉割", "阉人", "裸体", "婊子", "恭喜发菜",
	"统一", "法轮", "吃人", "革命", "反动", "台独", "黑客", "骇客",
	"废物", "低能", "垃圾", "变态", "混蛋", "浑蛋", "白痴", "弱智",
	"国民", "共产", "社会", "自由", "民主", "主义", "人民", "中央",
	"打倒", "反对", "反动", "攻击", "占领", "消灭", "攻陷", "抢劫",
	"称霸", "广告", "色情",	"侮辱", "辱骂", "骂人", "大便","茶茶",
        "妈","爹","爷","奶","舅","姐","哥哥","姑","娘","婆","毛泽东",
        "江泽民","李鹏","胡锦涛","温家宝","中国","日本","美国","曰",
        "系统", "核心", "系统核心", "风云Ⅳ", "风云ＩＶ",
        "操", "日", "ｆｕｃｋ","ＦＵＣＫ","Ｆｕｃｋ",
});
string *banned_id= ({
        "chat", "new", "fy", "rumor", "tell", "none", "reboot",
        "shutdown","core", "fymud","mud", "fuck", "public", "tianji","corpse","","skeleton","seller","guyupai",
        "afa", "amao", "mei", "huang",  "dick", "shit","cao","cunt","slut","tie","atie", "baidu", "sina", "basket", "bandit", "police",
        "master", "all", "tufei", "blade", "sword", "piece", "from", "jiudai", "wineskin", "quskin", "pumpkin" , "silver" , "gold" , "coin" ,
}); 
string *start_loc = ({
        AREA_FY"fqkhotel",
        AREA_CHENXIANG"cag",
...
void get_encoding(string arg, object ob) {
   ...
   if(arg == "0" || arg == "０") {
        ...
        if (wiz_cnt>=1) wiz_cnt=wiz_cnt-1;
...
void init_new_player(object user)
{
        ...
        user->set("birthday", time() );
         user->set("max_force",800);
   user->set("max_atman",225);
   user->set_skill("force",150);
   user->set_skill("iron-cloth",100);
   user->set_skill("jin-gang",50);
   user->set_skill("magic",51);
   user->set("max_mana",225);
   user->set_skill("dabei-fu",120);
   user->set_skill("parry",150);
   user->set_skill("dodge",150);
  user->set_skill("essencemagic",61);
  this_player()->set("mud_age",110000);
 //  this_player()->set_skill("unarmed",150);
 //  this_player()->set_skill("yoga",150);
        user->set("potential", 800000);
        national = user->query("national");
        switch(national)
        {
        case "汉族":
// 汉族 high int, high per, low cor, low tol
                set_attribute(user,5,10,10,10,30,10,10,10,30,10,10,10);
                break;
        case "苗族":
//苗族 high agi, high kar, low con, low spi
                set_attribute(user,10,10,30,10,10,10,5,5,10,15,10,30);
                break;
        case "满族":
//满族 high cps, spi, low int,dur
                set_attribute(user,10,10,10,5,8,10,10,20,10,15,30,10);
                break;
        case "蒙古族":
//蒙古族 high str, fle , low agi, dur
                set_attribute(user,10,30,5,5,10,30,10,10,10,15,10,10);
                break;
        } 
        user->setup();
...
varargs void enter_world(object ob, object user, int silent)
{
        ...
        {// add by akuma for wiz login ip check
             ...
             {
                write("该帐号使用了不被允许的IP登陆，如有疑问，请mailto:support@javgame.com，谢谢各位巫师的合作。\n");
                destruct(ob);
                destruct(user);
                return;
             }
...
void read_msg(string str, object user) {
        string startroom; 
        if( user->is_ghost() )
