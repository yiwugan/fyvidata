...
int exert(object me, object target)
{
        ...
        if ( extra < 100) return notify_fail("你的[清平内功］还不够纯熟！\n");
        if( !me->is_fighting() )
        ...
        for (i = 0 ; i< sizeof(enemy); i++) {   
                ...
                {       target = enemy[i];      
                   ...
                   msg = HIR +"$N双手转动，施展出［扭转乾坤］，企图用强烈的内力将$n的攻势引向$n。\n" NOR;
                    ...
                    if ((myexp*3/4+random(myexp)*4>yourexp ||!userp(me))&& !target->query_temp("is_unconcious")) {
                        ...
                        newweapon->unequip();
//      adding copy kee/gin/sen from the original 
                        ...
                        shadow->add_temp("apply/damage", extra);
