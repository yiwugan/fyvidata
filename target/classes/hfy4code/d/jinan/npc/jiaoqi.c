...
mixed next_data( string str, int i, string key );       //找出当前出牌者(id == str)的下 i 家Data里面的所有资料(key关键字)，
                                                        //这是最强大的一个函数！！！！！！！！！
...
varargs int do_dump(string str,object me);      //出牌
varargs int do_touch(string str,object me);     //摸牌
varargs int wash_mj(int amount);                //洗牌
varargs int do_win(string str,object me);   //胡牌
varargs int do_gon(string str,object me);       //杠牌
varargs int do_pon(string str,object me);       //碰牌
varargs int do_eat(string str, object me);      //吃牌
string check_id(string str,string kind);
string do_delete_Mj(string datastr,string str,int amount);      //字串处理函数，删掉一个牌
string show_mj(string str,int flag);    //将str用特殊的格式回显在屏上
string sort_data(string str);           //将str按特定的顺序排好
mixed do_check_num(string str,int flag);        //返回牌只str所代表的资料，可以是字串或数字 
//以下是PUBLIC变量的声明！！ 
//牌只资料
...
int Play;
int Times;
int Touched;    //用1和0表示是否已经摸了牌
int NowPlayer;
string List ="";
string LookP ="";
string *NowPlayerId = ({ "east com","south com","west com","" });       //正在进行游戏的人的ID，可以是NPC
string LastDumpId="";   //当前最后一个出了牌的人的ID
string LastDump="";     //当前最后出了的那张牌
string RestartId="";    //要求restart的人的ID 
//东家的所有资料！
mapping P1Data = ([
...
int PAUSE = 0;                  //判断是否处于暂停状态;PAUSE==1是为NPC叫的暂停，任何玩家都可以用continue命令取消；PAUSE==2
                                //是为玩家叫的暂停，只有该玩家才可用continue命令解除
string destop="";               //是桌面上所有的牌喔。
int DELAY =2;                   //NPC吃碰杠胡后出牌的延时秒数
string TIMES = "";              //看东南西北风的  
void create()
{
        set_name("焦七太爷",({"jiaoqi", "jiao"}));
        set_weight(1);
        set("gender", "男性");
        set("age", 72);
        set("long",
                "一个乾枯瘦小的小老头，一张乾瘪蜡黄的脸上，长着双小小的三角眠，留者几根稀疏
的山羊胡子，花花的头发，几乎已快掉光了。这个其貌不扬的小老头，就是名震南七
北六十三省的赌王。焦七太爷平生大赌小赌不下千万次，据说连一次都没有输过至少
在三十岁以后就没有输过。焦七太爷不但赌得精，眼睛更毒，不管大郎中小郎中玩票
的郎中还是郎中的专家，从来没有人敢在他面前玩一点手法，因为不管你用什么手法，
焦七太爷一眼就可以看出来。你可以问他关于麻将的问题（ask jiaoqi about mj）。\n"
            );
...
void greeting(object ob)
{
        ...
        switch( random(10) ) {
                case 0:
                        say( "焦七太爷用一双留着叁寸长指甲的手，捧起个纯银水烟壶“呼噜呼噜”，先抽了两口，
才朝"+ob->query("name")+"笑了笑，道：这位" + RANK_D->query_respect(ob)+ "“坐，请坐。”\n");
                        break;
                case 1:
                        say( "焦七太爷轻轻的笑道：“听说这位"+RANK_D->query_respect(ob)+"近来的手气不错？”\n");
                        break;
                case 2:

                        say( "焦七太爷显得很满意，道：“很好，赌钱不但是种很大的学问也是种享受！”\n");
                        break;
        }
        return; 
} 
int do_addin(string arg)
{
        int amount, total;
        string money_type;
        string location, c_loc;
        object me, gold;
        //object me=this_player(), money = present(money_type+"_money", me);
        //sscanf(arg, "%d %s", amount, money_type);
        me=this_player();
        gold = gold=present("gold_money", me); 
        ...
        ||  me->query("id")==P4Data["Id"]){
            if (!Playing) message_vision("$N对着欠脚啊～～不停的叹着气。\n",me);
            else message_vision("$N对着大家说今天自摸"+(random(12)+1)+"次就好。\n",me);
            return 1;
        }
       if (Playing && P1Data["Id"]!="east com"){
                if (random(1))
                message_vision(this_object()->name()+
                "对着"+me->name()+"说道：牌局正在进行中请到旁边坐一下...。\n",me);
                else  message_vision("$N对着大家说：“老夫我也想玩一把～～”不停的叹着气。\n",me);
                return 1;
        } 
//以下代码为风云工作室而加入（押上银两进行游戏）
/*
        if(!gold) return notify_fail("你身上没有金子呀。\n");
        total = gold->query_amount();
        if((int) gold->query_amount() < 10)
                return notify_fail("最少要有十两金子才可进行麻将游戏。\n");*/
...
//      if ((!P4Data["Id"] || P4Data["Id"]=="") || ( P2Data["Id"]=="south com") || ( P3Data["Id"]=="west com")
//              ||( P1Data["Id"]=="east com")) {
//      if(P1Data["Id"]=="east com") {
//              gold->add_amount(-1);
//              message_vision("$N从身上拿出一两金子放在桌面上作为底金。\n",me);
//      }
        me->set_temp("playing_mj",1); 
        if (!P4Data["Id"] || P4Data["Id"]==""){
                 P4Data["Id"]=me->query("id");
//               P4Data["Money"]=total;//为风云工作室而加入（押上银两进行游戏）
...
//         P2Data["Money"]=total;//为风云工作室而加入（押上银两进行游戏）
...
//               P3Data["Money"]=total;//为风云工作室而加入（押上银两进行游戏）
                 message_vision("$N加入了这场牌局。\n",me);
...
//               P1Data["Money"]=total;//为风云工作室而加入（押上银两进行游戏）
                message_vision("$N加入了这场牌局。\n",me);
        ...
        else return notify_fail("人数已足。\n"); 
} 
int do_restart(string str)
{
        object me=this_player();
        if( !present(P4Data["Id"]) || !present(P2Data["Id"]) || !present(P3Data["Id"]) ||
                !present(P1Data["Id"]) ) ;//只要有一个玩家离开，任何人都可以restart!!
        if( str=="karry@FengYun_MJ") ;
        else {
        ...
        //当有两个玩家以上进行游戏时，游戏者就不能随便restart
        //      write( RestartId+"  **\n");
        //      if( !present(P4Data["Id"]) || !present(P2Data["Id"]) || !present(P3Data["Id"]) || !present(P1Data["Id"]) ) ;
                if( (me->query("id")==RestartId || RestartId=="") && P2Data["Id"]!="south com" ) {
                        message_vision(HIY+me->query("id")+NOR+" 要求把牌局重新设定或离开，这需要得到至少另外一个人的同意，如果你同意，请键入restart。\n",me);
                        RestartId=me->query("id");
                        return 1;
                }
        }
/*        //在这里将底金还给玩家 for 风云
        if(P1Data["Id"]!="east com" && present(P1Data["Id"])&& P1Data["Money"]>0 ) present("gold_money", present(P1Data["Id"]))->add_amount(1);
        if(P2Data["Id"]!="south com" && present(P2Data["Id"])&& P2Data["Money"]>0 ) present("gold_money", present(P2Data["Id"]))->add_amount(1);
        if(P3Data["Id"]!="west com" && present(P3Data["Id"])&& P3Data["Money"]>0 ) present("gold_money", present(P3Data["Id"]))->add_amount(1);
        if(present(P4Data["Id"])&& P4Data["Money"]>0) present("gold_money", present(P4Data["Id"]))->add_amount(1); 
...
int do_dump(string str,object me)
{
        ...
        if( !present(P4Data["Id"]) || !present(P2Data["Id"]) || !present(P3Data["Id"]) ||
                !present(P1Data["Id"]) ) {
                message_vision("$N说道："+HIY+"有人擅自离开了！！！"+NOR+"请用"+HIW+"addin"+NOR+"命令重新加入。\n",this_object());
                do_restart("karry@FengYun_MJ");
        } 
        if(set_eval_limit(-1)<700000 ) reset_eval_cost();       //防止出现“Too long evaluation...”的错误
        ...
        if(present(NowPlayerId[NowPlayer]) ) tell_object(present(NowPlayerId[NowPlayer]), "焦七太爷对你说：现在轮到你摸牌。\n"); 
        if (Auto_Tin){
        ...
        if (count==(end-16-Goned)){
           ...
           str="所剩下的牌只有:\n"+show_mj(List[count..end+1],0)+"\n";
           for (i=0;i<Play;i++) str+= NowPlayerId[i]+" :\n"+show_mj( next_data(NowPlayerId[i], 0, "Mj"), 0);
...
{
        int i,j;
        int RESTART;//为风云工作室加入，判断玩家输光时重新开始
        int *winner=({0,0,0,0});
        string Mj="";
        mapping who, who1;
        object met;
...
"@b" : ({ "海底摸月", 1 }),"@c" : ({ "海底捞鱼", 1 }),"@d" : ({ "平胡", 1 }),"@e" : ({ "东风东", 2 }),
"@f" : ({ "南风南", 2 }),"@g" : ({ "西风西", 2 }),"@h" : ({ "北风北", 2 }),"@i" : ({ "全求人", 2 }),
"@j" : ({ "叁暗刻", 2 }),"@H" : ({ "二丕口", 2 }),"@k" : ({ "叁杠子", 2 }),"@B" : ({ "春夏秋冬", 2 }),
"@C" : ({ "梅兰菊竹", 2 }),"@l" : ({ "门清自摸", 2 }),"@m" : ({ "碰碰胡", 2 }),"@n" : ({ "混一色", 2 }),
"@o" : ({ "小叁元", 4 }),"@p" : ({ "四暗刻", 4 }),"@q" : ({ "清一色", 4 }),"@r" : ({ "字一色", 4 }),
"@s" : ({ "七对子", 4 }),"@t" : ({ "大叁元", 4 }),"@u" : ({ "五暗刻", 4 }),"@F" : ({ "小四喜", 4 }),
"@D" : ({ "七抢一", 4 }),"@E" : ({ "绿一色", 8 }),"@v" : ({ "大四喜", 8 }),"@w" : ({ "天胡", 8 }),
"@x" : ({ "地胡", 8 }),"@y" : ({ "人胡", 4 }),"@z" : ({ "国士无双", 8 }),"@A" : ({ "八仙过海", 8 }),
"@J" : ({ "清幺九", 2 }),"@I" : ({ "混幺九", 1 }),
]);//H
           get_tc(who);
////////////////
...
//           if (Count){
//              str+=sprintf("\t\t\t%-12s%8s 台\n","连"+chinese_number(Count)+"拉"+chinese_number(Count),chinese_number(Count));
//              j+=Count;
//           }
//           j+=1; 
...
/////////////
           if (!Touched){
              if (Play==4){
                 ...
                 winner[1]+=j;
                 who["winner"][1]+=j;
//               who["Money"]+=j;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000
...
/*
//                  who1["Money"]-=j;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000
//                  if(who1["Money"]<=1 ) {
                        RESTART=1;//输光就要踢出去
                        if(present(who1["Id"]) ) present(who1["Id"])->add("deposit", who1["Money"]*10000);
                        destruct(present( "gold_money", present(who1["Id"])));
                        who1["Money"]=0;
                        //who1["Id"]="";
                    }
  */
           ...
           if (Play==4){
              ...
              if (NowPlayerId[NowPlayer]==who["Id"]&&Touched){
                 if (who["Out"]==""&&who["OutPon"]==""){
                    winner[1]-=24;
                    who1["winner"][1]-=24;
//                  who1["Money"]-=24;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000 
                    j=-24;
                 }else {
                    winner[1]-=18;
                    who1["winner"][1]-=18;
//                  who1["Money"]-=18;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000
                    j=-18;
                 }
              }else{
                 if (who["Out"]==""&&who["OutPon"]==""){
                    winner[1]-=6;
                    who1["winner"][1]-=6;
//                  who1["Money"]-=6;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000
                    j=-6;
                 }else{
                    winner[1]-=5;
                    who1["winner"][1]-=5;
//                  who1["Money"]-=5;//为风云工作室而加入（押上银两进行游戏），换算1两金等于10000
                    j=-5;
                 }
/*
                 if(who1["Money"]<=1) {
...
/////////////
        ...
        for (i=0; i<33; i++) Mj+="  ";
        Mj+="  \n";
        Mj+="        "+HIW+"英文名字      "+HIW+"中文名字  已玩局数  战  绩  自摸次数  放枪次数  \n";
        for (i=0;i<4;i++){
        ...
        Mj+="  ";
        for (i=0; i<33; i++) Mj+="  ";
        Mj+="\n"+HIW+"要查阅总战绩请用top命令。 ——【风云工作室】\n"+NOR;
        if (Play==4) tell_room(environment(this_object()),Mj);
       /*FOR 风云
        if( userp(present(P1Data["Id"]))&& P1Data["Money"]>1 ) present("gold_money", present(P1Data["Id"]))->set_amount(P1Data["Money"]-1);
        if( userp(present(P2Data["Id"]))&& P2Data["Money"]>1 ) present("gold_money", present(P2Data["Id"]))->set_amount(P2Data["Money"]-1);
        if( userp(present(P3Data["Id"]))&& P3Data["Money"]>1 ) present("gold_money", present(P3Data["Id"]))->set_amount(P3Data["Money"]-1);
        if( present(P4Data["Id"])&& P4Data["Money"]>1 ) present("gold_money", present(P4Data["Id"]))->set_amount(P4Data["Money"]-1);
       ******** 
        ******以下语是在每打完一圈之后就重新开始，防止系统总是以为函数调用太深*******
*/
        if( (Times-1) / 4 == 3 && NowPlayerId[1]=="east com" && NowPlayerId[0]!=who["Id"]){
                message_vision("$N说道："+HIY+"一圈打完～～～"+NOR+"请用"+HIW+"addin"+NOR+"命令重新加入。\n",this_object());
                new(__DIR__"jiaoqi")->move(environment(this_object()) );
                destruct(this_object());
                return 1;
        }
        //*****************************************************************************
        if( RESTART==1 ) {
                message_vision("$N说道："+HIY+"有人输光了！！！"+NOR+"请用"+HIW+"addin"+NOR+"命令重新加入。\n",this_object());
                do_restart("karry@FengYun_MJ");
        }
        call_out( "wash_mj", 10, Play);
        return 1;
} 
int do_check_win(string str)
{
        ...
        if(size>27) {
                ...
                if(Sp==7) {
                        Tc+="@s";//7对子
                        Tc = do_delete_Mj(Tc, "@d", 1);//是七对子就不是平胡了
                        return 1;
                }
        } 
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        ...
        //玩家取牌一次取13只:P
        P1Data["Mj"]=List[0..25];//"9t9t9teaea3t4t5t2s7s";P1Data["Out"]="2t3t4t";
        P2Data["Mj"]=List[26..51];//"1s2s3s4s5s9s3t";P2Data["Out"]="1w2w3w4w5w6w";//
        P3Data["Mj"]=List[52..77];
        P4Data["Mj"]=List[78..103];
//        P4Data["Mj"]="1w1w1w2w3w4w5w6w7w8w9w9w9w";//List[78..103];//
...
mixed do_check_num(string str,int flag)
{
        ...
        if(number>44) tell_object( present("wiman"), number+" "+str1+"\n");
...
void dump_what( object me )
{
        ...
        who["H2Es2"] ="";
        who["H1s"] =""; 
        if(set_eval_limit(-1)<700000 ) reset_eval_cost();       //防止出现Too long evaluation的错误 
        switch( who["Attitude"] ) {
//以下是最cheap的打法，能胡就行
case "cheap" :
...
int do_continue(object me)
{
        ...
        //if(!Touched) {
                if( !userp(ob)&&PAUSE!=2 ) {
                        ...
                        if(!Touched) { do_touch( 0, ob );return 1; }
                        else { dump_what( ob ); return 1; }
                }
                if( who["Pause"]=="yes" ) {
                        PAUSE =0; who["Pause"] = "";
...
string filt_mj(string strW, object me)
{
        ...
        who["1s"] += strW; 
           //who["1s"]+" "+who["2Ps"]+" "+who["2Es1"]+" "+"filt完毕\n" 
           if (strW!="") return strW;
           return "";
} 
int filt_hu_color(string strW, object me)
...
string find_mj(string str, string mj)
{
        ...
        // str1+str2+str3+str4+" find_mj here?
        if(str4!="") str0 = str4;
