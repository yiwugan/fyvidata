...
//        else{tell_object(me,"对方的辈分不比你低！\n");return 1;}
        ...
        if( (object)ob->query_temp("pending/join") == me ) { 
                ...
                "$N跪了下来向$n恭恭敬敬地磕了一个响头，叫道：「大哥！」\n\n",
                        ...
                        ob->add("organization/betrayer", 1);
                } else 
                        message_vision(
                                "$N决定收$n为成员。\n\n"
                                "$n跪了下来向$N恭恭敬敬地磕了一个响头，叫道：「大哥！」\n",
                ...
                tell_object( ob, sprintf("恭喜您成为%s的第%s代成员。\n", organization["org_name"],
                        chinese_number(organization["generation"]+1) )); 
                ob->set("organization/org_name",organization["org_name"]);
                ob->set("organization/leader_name",me->query("name"));
                ob->set("organization/generation",me->query("organization/generation")+1);
                return 1;
        } else { 
                old_enr = me->query_temp("pending/enroll");
                if( ob==old_enr )
