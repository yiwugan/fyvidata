...
//    if( me->is_member_of(ob) || ob->query("organization/org_name") == me->query("organization/org_name") ) {
    if( me->query("organization/org_name") == ob->query("organization/org_name")&&(int)me->query("organization/generation") > (int)ob->query("organization/generation")) {
        message_vision("$N恭恭敬敬地向$n拱手请安，叫道：「大哥！」\n", me, ob);
                if (!userp(ob)) ob->org_rerank(me);
        return 1;
    }
//    else {tell_object(me,"你的辈分比对方高！");return 1;}
    ...
    if( (object)ob->query_temp("pending/enroll") == me )
    {
            ...
            (string)ob->query("organization/org_name"))
        {
            ...
            me->add("organization/betrayer", 1);
        }
        ...
        printf("恭喜您成为%s的第%s代成员。\n", ob->query("organization/org_name"),
               chinese_number( ob->query("organization/generation")+1));
               me->set("organization/leader_name",ob->query("organization/name"));
               me->set("organization/org_name",ob->query("org_name"));
               me->set("organization/generation",ob->query("organization/generation")+1);
        return 1;
