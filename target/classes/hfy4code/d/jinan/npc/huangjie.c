...
// we think this player will be okey to take a seat at this moment, so, record all the info and take his money!
        ...
        message_vision("$N从身上拿出银子放在桌面。\n（table）可以查看桌面情况，不玩的时候请（leave）。\n赌（bet）或者放弃（fold）之前请看牌（cards）。 \n",me);
        //message_vision("$N从身上拿出"+ chinese_number(amount)+money->query("base_unit")+money->name()+"放在"+c_loc+"。\n",me);
        me->start_busy(1);
