...
void create()
{
        set_name("本观", ({ "master benguan", "master"}) );
...
大理天龙寺高僧。\n"
        ...
        set("chat_msg_combat", ({
                (: perform_action, "duan-yiyangzhi.zhongchong" :)
        }) );
...
{
        if(me->query_temp("won_benxiang") && ( me->query("gongxian") > 34560 ))
        {
        command("smile");
...
{
        if(me->query("family/family_name") == query("family/family_name") && me->query("family/master_id") == query("id"))
        {
