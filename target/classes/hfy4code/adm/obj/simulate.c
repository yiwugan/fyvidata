...
/* add by cbra for disable_command bugs */
/* part 1 of total 2 part */

        if (current_object->super) {
            if (current_object->super->flags & O_ENABLE_COMMANDS) {
                command_giver=current_object->super;
                (void) apply(APPLY_INIT, current_object, 0, ORIGIN_DRIVER);
	    }
            command_giver=current_object;
            (void) apply(APPLY_INIT, current_object->super, 0, ORIGIN_DRIVER);

            if (current_object->super->contains) {
                for (ob = current_object->super->contains; ob; ob = next_ob) {
                     next_ob = ob->next_inv;
	             if (ob==current_object) continue;	
                     command_giver = current_object;
        ...
        if (current_object->contains) {
            for (ob = current_object->contains; ob; ob = next_ob) {
                next_ob = ob->next_inv;
                command_giver = ob;
                (void) apply(APPLY_INIT, current_object, 0, ORIGIN_DRIVER);
                command_giver = current_object;
...
/* end of part 1 of total 2 part by cbra */
...
/* end of part 2 of total 2 part by cbra */
