...
void create()
{
	set_name("风云变幻新闻版", ({ "newsboard"}) );
    set("location", "/d/wiz/news"); // 这里用了一个ROOM来放置BOARD，简单，你做 
        set("long",     "专门用来写新闻的版。\n" );
	set("board_id", "fybhnews_b");              // 成一个指令来输入新闻也可以。
	setup();
	set("capacity", 500);
}
...
int check_news(object me)
{
 	...
 	if(userp(me))
	{
		...
		if(i>10) write("你可以 Help news 来查看风云变幻新闻系统的使用方法。\n"); // 提醒不知道有NEWS系统的玩家使用news
	}
}
...
int main(object me, string arg)
{
	...
	if( !pointerp(notes) || !sizeof(notes) )
		return notify_fail("「风云变幻」目前没有任何新闻。\n");
		
	if( !arg ) 
	{
		if(i>10) write("你可以 Help news 来查看风云变幻新闻系统的使用方法。\n"); 
			return notify_fail(
				"「风云变幻」目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
	...
	"-------------------------------------------------------------------------------\n",
		BBLU HIG"「"HIR"刀"HIY"剑"HIW"笑"HIG"」第"+chinese_number(num + 1)+"条新闻"NOR, 
