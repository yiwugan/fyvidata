...
void create()
{
        set_name("本尘", ({ "master benchen", "master"}) );
...
大理天龙寺高僧。\n"
        ...
        set("chat_msg_combat", ({
//                (: perform_action, "duan-yiyangzhi.shaoze" :),
                (: perform_action, "duan-yiyangzhi.guanchong" :)
        }) );
        ...
        set_skill("liumaishenjian", 40);
        set_skill("duan-force", 200);
        set_skill("duan-yiyangzhi", 230);
...
{
        if(me->query_temp("won_bencan") && ( me->query("gongxian") > 34560 ))
        {
        command("smile");
