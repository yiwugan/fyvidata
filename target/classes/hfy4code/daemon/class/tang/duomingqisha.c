...
int perform(object me, object target) {
        ...
        if (me->query_skill("tangsi",1)<50) 
                return notify_fail("你的「唐诗十六式」不够熟练！\n");
        
        if( !target ) target = offensive_target(me);
        if(!objectp(target) || !target->is_character() || !me->is_fighting(target)) {
                return notify_fail("「夺命七煞」只能对战斗中的对手使用。\n");
        }
 ...
 msg = HIC"$N"+HIC"最后孤注一掷，第六、第七式连环施出，点向"+"$n"+HIC"期门、立关两处要穴。\n"NOR;
        bonus = ((int) random(me->query("combat_exp")/40*bonus)+me->query("combat_exp")/4)/((int) target->query("combat_exp")+1);
                ...
                msg += YEL"$n"+YEL"就地一滚堪堪躲过了$P"+YEL"的攻击！！\n"NOR;
                message_vision(msg, me, target);
                me->start_busy(2);
                }
