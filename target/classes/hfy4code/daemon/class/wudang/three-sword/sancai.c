...
int perform(object me, object target)
{
        ...
        if ((me->query("class")!="wudang")&&(me->query("class")!="ghost"))
                return notify_fail("只有真武派系的弟子才可以发动叁才剑阵。\n");       
...
int single_perform(object target, object me) {
        ...
        max_exp=me->query("combat_exp")+random(me->query("combat_exp")/2);
        ...
        for(i=0;i<n;i++)
        {
				if(i==2||i==3) skill += skill*n/4;
				if(i==4) skill += skill*n/2;
                msg = HIR +  style[i] + NOR;
