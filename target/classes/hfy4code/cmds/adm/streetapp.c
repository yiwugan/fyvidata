...
#define __ROOMDIR__ "/p/residence/"
inherit F_CLEAN_UP;
int main()
{  string filename,file;
   ...
   if(room->query("coor/z")!=0) return notify_fail("你只能扩展风云街！！\n");
   if(room->query("coor/x")!=30) return notify_fail("你只能扩展风云街！！\n");
   if(room->query("coor/y")<2050) return notify_fail("你只能扩展风云街！！\n");
   i=(room->query("coor/y")-2050)/10;
...
filename=__ROOMDIR__"street"+(string)(i-1)+".c";file=read_file(filename);
   ...
   message_vision("嘁哩嚓哪还真出来一片空地，风云街看起来又长了一些。\n",me);
return 1;
}
