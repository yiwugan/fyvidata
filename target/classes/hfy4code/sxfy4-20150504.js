...
function chineseShiToNum(cn){
    var cnShis = '子丑寅卯辰巳午未申酉戌亥';
 
    if (!cn) {
        return 0
    }
	return cnShis.indexOf(cn);
}
...
var listQuestGhost=new Array();
var listQuestCancel=new Array();

function initQuestAsk(){
	listQuestCancel=new Array();
	
   var fso, f1, ts, s;
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questCancel.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
	   if (s.indexOf("//")==-1)
			listQuestCancel.push(s);;
	   s = ts.ReadLine();
   }
   ts.Close();
   world.note("---> loading mapQuestCancel completed");
}
...
//	if (qpath.indexOf("goo ")<0)
//		qpath="north;"+qpath;
	var moves=qpath.split(";");
	var move="";		
	for (var i=0;i<moves.length;i++){
//	world.note("--->move:"+moves[i]);
...
//			move="";
		...
		}else if (moves[i].indexOf("auto")>=0 || moves[i].indexOf("quest")>=0){
			if (move!=""){
...
function generateQuestPostMove(){
	addMove2("get corpse;umk");	
//	addMove2("goo illusionist nan");	
//	addMove2("checkBusy");	
	addMove2("north;enter;questDoneOrGone;questReturnAfterDone");
	addMove2("askQuestReturn");
	addMove2("out;south;checkStatus");	
	addMove2("checkQuestItem");
	addMove2("west;openBox");	
	addMove2("east;questList");
...
function queryQuestPath(cname){
	...
	if (ret!=null)
		world.setvariable("varQuestType","kill");
	else{
		ret=queryQuestItemPath(cname);
		if (ret!=null)
			world.setvariable("varQuestType","item");
		else
...
function queryQuestKillPath(cname){
	...
	if (ret!=null && npcEname!=null){
		...
		ret=ret.replace(/;kill2 /g,";questKillOther ");
		ret=ret.replace(/;kill /g,";questPoke ");
		ret=ret.replace(/;fight /g,";questPoke ");
		ret=ret.replace(/;stealkill /g,";questPoke ");
		ret=ret.replace(/;askkill /g,";questPoke ");
	}
...
function loadQuestNpcGhostMap(){
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questNpcGhost.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
...
function loadQuestNpcKillMap(){
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questNpcKill.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
	   var listLine=s.split(",");
	   if (s.indexOf("//")==-1)
...
function loadQuestItemKillMap(){
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questItemKill.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
	   var listLine=s.split(",");
	   if (s.indexOf("//")==-1){
...
function loadQuestItemGetMap(){
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questItemGet.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
	   var listLine=s.split(",");
	   if (s.indexOf("//")==-1)
...
function loadQuestItemKillAdditionalMap(){
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questItemKillAddtional.txt", ForReading);
   s = ts.ReadLine();
   while (!ts.AtEndOfStream){
	   var listLine=s.split(",");
	   if (s.indexOf("//")==-1){
...
function loadQuestNpcLocationMap(){
   var fso, f1, ts, s;
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\questNpcKill.txt", ForReading);
   ...
   while (!ts.AtEndOfStream){
	   var listLine=s.split(",");
	   if (s.indexOf("//")==-1){
		   mapNpcLocation[listLine[1]]=listLine[6];
	   }
	   s = ts.ReadLine();
   }
   ts.Close();
   world.note("---> loading mapNpcLocation completed");
}
...
function loadMissionNpcMap(){
   ...
   while (!ts.AtEndOfStream){
	   ...
	   if (s.indexOf("//")==-1){
		   ...
		   mapMissionNpcStatus[listLine[1]]=listLine[2];
		}
	   s = ts.ReadLine();
   }
   ts.Close();
   world.note("---> loading mapMissionNpc completed");
   world.note("---> loading mapMissionNpcStatus completed");
}
...
function queryMissionNpcStatus(ename){
	return mapMissionNpcStatus[ename];
}

function isDangerousMissionNpc(ename){
	var status=mapMissionNpcStatus[ename];
	if (status!=null && status.indexOf("ad")>=0)
		return true;
	else return false;
}

function isStealKillNpc(ename){
	var status=mapMissionNpcStatus[ename];
	if (status!=null && status.indexOf("st")==0)
		return true;
	else return false;
}

function isMihunKillNpc(ename){
	var status=mapMissionNpcStatus[ename];
	if (status!=null && status.indexOf("mihun")>=0)
		return true;
	else return false;
}

function getNpcStealItem(ename){
	var status=mapMissionNpcStatus[ename];
...
function generateAutokillPerform(pfm){
	...
	var w=world.getvariable("varUseWhistle");
	var ret=""
	if (t==""){
		ret=pfm.replace(" @","");
		ret=ret.replace(" @","");
		...
		if (w=="yes"){
			world.note("---> abort autokill with whistle");
			ret="bluebird";
		}else{
...
function loadLocationMapForQuest(){
	mapLocationNameId={};
	mapLocationIdPath={};
	listLocationId=new Array();
   ...
   var ForReading = 1;
   fso = new ActiveXObject("Scripting.FileSystemObject");
   ts = fso.OpenTextFile("C:\\mud\\fyvi\\allLocationQuest.txt", ForReading);
   s = ts.ReadLine();
...
function parsePathActionForMission(path,area){
	...
	if (wimpy99==true)
		ret.unshift("wimpy 99");	
	return ret.join(";");
}
//

//quest path
function parsePathActionForQuestByLocId(locId){
	var path=queryMovePathById(locId);
	var areaSep=locId.indexOf("/");
	var area=locId.substr(0,areaSep);
	var ret=new Array();
	var paths=path.split(";");
	var j=0;
	for (j=0;j<paths.length;++j){
		...
		if (move=="m1" || move=="m2" || move=="m3" || move=="m4" )
			...
			ret.push(move.replace("k ","questKillOther "));
		else if (move=="p"){
			//skip poke action inside path
		}
		else if (move.indexOf("po ")==0)
			ret.push(move.replace("po ","questPokeOther "));
		else if (mapPathMoveMapping[move]!=null && mapPathMoveMapping[move]!="")
			ret.push (mapPathMoveMapping[move]);
		else
			ret.push (move);
...
//			move="";
		...
		}else if ( moves[i].indexOf("#")>=0 || moves[i].indexOf("ggMyItem")>=0){
			if (move!=""){
				addMove2(move+";whisper");
				addMove2(moves[i]+";whisper");
				move="";
	...
	if (move!="")
		addMove2(move+";whisper");
	var lastMove=listMoves.pop();
	lastMove="wimpy 99;"+lastMove;
	addMove2(lastMove);
}
...
//time function
function convertChineseTimeToNumber(cTime){
//九分二十四秒
	var p=cTime.indexOf("分");
	var cm=cTime.substr(0,p);
	var cTime2=cTime.substr(p+1);
	p=cTime2.indexOf("秒");
	var cs="";
	if (p>0)
		cs=cTime2.substr(0,p);
	var m=chineseToNum(cm);
	var s=0;
	if (cs!="")
		s=chineseToNum(cs);
	return m*60+s;
...
function increaseVariable(varname){
	var v=world.getvariable(varname);
	if (v !=null){
		v=parseInt(v)+1;
		world.setvariable(varname,v);
	}
}
