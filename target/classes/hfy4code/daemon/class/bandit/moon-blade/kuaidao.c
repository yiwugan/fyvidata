...
int perform(object me, object target)
{
        ...
        if (exp > 200000000 ) exp = 200000000 ;
        extra = me->query_skill("blade") + me->query_skill("move") + n*100;
		bonus = me->query("combat_exp")/1000*extra/8000;
		
		 if ( me->query("class")!= "bandit")extra = extra/3;
		 if ( me->query("class")!= "bandit")bonus = bonus/3;
			
        if((int)me->query_skill("moon-blade",1) > 120) extra = extra + bonus;
        for(i=0;i<num;i++){
		target = enemy[i];
		if( target->query("kuaidao_boss"))
			message_vision(HIY"但$n"+HIY"抢上半步，躲过了$N"+HIY"这一刀！\n"NOR,me,target);
                else if(target->query("combat_exp") < random(exp)*10+exp/3) {
