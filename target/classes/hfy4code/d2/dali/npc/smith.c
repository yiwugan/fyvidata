...
void create()
{
        ...
        set("gender", "男性" );
        set("age", 42);
        set("long","段铁匠一身打铁功夫名不虚传，他打出的铁器经久耐用．\n");
        set("combat_exp", 5);
...
void greeting(object ob)
{
        ...
        switch( random(5) ) {
                case 0:
                        say( "段铁匠粗声粗气地说道：这位" + RANK_D->query_respect(ob)
                                + "，要什么？\n");
                        break;
                case 1:
                        say( "段铁匠满脸汗水说道：这位" + RANK_D->query_respect(ob)
                                + "，我太忙了，帮不了你。\n");
                        break;
        }
}
...
int do_ding(string arg)
{
	...
	return notify_fail("你现在只能拥有"+chinese_number((int)me->query("age")/11)+"件自造物品。\n");
...
{
  object xzhihuan,gold1,ob;
if(!arg || arg !="zhihuan")
  return notify_fail("你要修什么？！\n");
gold1= present("gold_money", this_player());
if(!gold1) return notify_fail("你身上没有金子。\n");
if((int) gold1->query_amount() < 5)
  return notify_fail("你身上没有那麽多金子。\n");
if(!objectp(ob=present("zhihuan",this_player())))
  return notify_fail("你身上没有指环。\n");
command("say 这个指环应该修一下。\n");
xzhihuan = new("/questobj/obj/xzhihuan");
xzhihuan->move(this_player());
	gold1->add_amount(-5);
        write("你将身上的"+ob->name()+"交给"+name()+"，修复成一个精美的"HIC + ob->name()+NOR"。\n");
        destruct(ob);
        return 1;
}
