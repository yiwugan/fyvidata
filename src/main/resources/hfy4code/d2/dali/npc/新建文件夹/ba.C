...
int give_him()
{
	...
	me = this_player();
        if(me->query("class") != "dali") 
           {
         message_vision("$NËµµÀ£ºÄã²»ÊÇ´óÀí¹ÙÔ±£¬²»ÄÜÉêÇëÕâÑùµÄÈÎÎñ£¡\n",this_object(), me);
         return 1;
            }
        if(me->query("gongxian") < 10000) 
      {
         message_vision("$NËµµÀ£ºÄãµÄ¹±Ï×²»¹»À²£¬²»ÒªÉêÇëÕâÃ´À§ÄÑµÄÈÎÎñ£¡»¹ÊÇÈ¥×öÒ»Ğ©¼òµ¥µÄ°É¡£\n",this_object(), me);
         return 1;
        }
...
message_vision("$NËµµÀ£º$n²»ÊÇÄÃÒ»°ÑÒ©³úÁËÂğ£¿¡£\n",this_object(), me);
     return 1;
     }
       obj = new("/d/dali/obj/yaochu");
message_vision("$NËµµÀ£º¼ÈÈ»Äã¶ÔÍÚÒ©¸ĞĞËÈ¤£¬ÎÒ¾Í°ÑÕâÏîÖØÒªµÄ¹¤×÷½»¸¶¸øÄãÁË£¬°ÍÌìÊ¯¸øÁË$nÒ»°ÑÒ©³ú¡£\n",this_object(), me);
    obj->move(me);
...
//        chinese_number(score) + "µã×ÛºÏÆÀ¼Û¡£\n" 
        ...
        tell_object(who,"°ÍÌìÊ¯ËµµÀ£º´óÀí¹ú¿â»áĞèÒªÕâÖÖ¿İÖ¦ÀÃ²İÂğ£¿\n");
        return 0;
}
