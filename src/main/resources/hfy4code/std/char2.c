...
void heart_beat()
{
	...
	{
    	if(stringp(savemyass = (string) query("env/savemyass")) && ! query_temp("netdead") && query("env/savemyass") != "quit" )
		command(savemyass);
	else
	...
	if(this_object()->query_temp("using_netscape"))
	COMBAT_D->netscape_report_status(this_object());
...
int visible(object ob)
{
	...
	if( intp(invis) && (invis > lvl) ) {
		if( !wizardp(this_object()) && !wizardp(ob) && this_object()->query_temp("apply/astral_vision") )
		{
			if( msk >= ob->query_skill("magic")) 
				return 1;
			}
		return 0;
	}
