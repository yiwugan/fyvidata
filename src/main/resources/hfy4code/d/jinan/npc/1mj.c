...
int dump_samecolor( string str_out, string str_mj, object me, mapping );//做清一色
...
void create()
{
       ...
       set("age", 18);
        set("long",
                ...
                现在她流落民间，你可以问她关于麻将的问题（ask bai about mj）。\n"
                );
...
/////////////
        ...
        for (i=0; i<33; i++) Mj+="  ";
        Mj+="  \n";
        Mj+="        "+HIW+"英文名字      "+HIW+"中文名字  已玩局数  总战迹  自摸次数  放枪次数  \n";
        for (i=0;i<4;i++){
...
//        for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
        ...
        //玩家取牌一次取16只:Q
        if (Play>0) P1Data["Mj"]=List[0..25];//"1t1t1t9t9t9teaeaea3s4s5s8s";//P1Data["Out"]="1w2w3w4w5w6w";
        if (Play>1) P2Data["Mj"]=List[26..51];//"1s2s3s4s5s9s3t";P2Data["Out"]="1w2w3w4w5w6w";//
        if (Play>2) P3Data["Mj"]=List[52..77];
        if (Play>3) P4Data["Mj"]=List[78..103];//                "1w2w3w3w3w5w5w5weaeaea1s8s";//
        count=(Play*2*13);
...
int get_tc(mapping who)
{
        ...
        if( check==0 ) {//不能有一九            
            ...
            //if (do_check_num(str[i..i+1],1)!=1&&do_check_num(str[i..i+1],1)>8) check++;
                ...
                if(check==0) { Tc+="@Z"; tell_object( present("wiman"), "Tc+= @Z \n"); }
        }
...
//以下是清一色
        if( dump_samecolor( (who["Out"]+who["OutGon"]+who["OutPon"]), who["Mj"], me, Temp ) ) { 
                who["Hu_type"] = "samecolor";
                break;
        }
//从这以下是混一色
        else if( dump_twocolor( (who["Out"]+who["OutGon"]+who["OutPon"]), (Temp["tempW"]+Temp["tempT"]+Temp["tempS"]), me, Temp ) ) {
                who["Hu_type"] = "twocolor";
...
int npc_action( object me )
{
                        ...
                        if( !userp(ob) ){
                                if(who["Attitude"]=="cheap") {
                                        ...
                                        if(get_times(who)>3) {                                          
                                                ...
                                                do_win( 0, ob );
                                                return 1; }
                                        //else tell_object( present("wiman"), "it is too cheap!\n");
        ...
        //for(i=0; i<3; i++) {
                ...
                //ob = next_data(me->query("id"), i+1, "player");
                if( !userp(ob) ) {
                        if( who["Hu_type"]!="pinhu") {
                                if( do_check_Mj(str, LastDump)>1 ){
                                        //PAUSE = 1;
                                        do_pon( LastDump, ob );
                                        return 1;
                                }                       
                        } else if( who["Hu_type"]!="twocolor" || who["Hu_type"]!="samecolor" ) {
                                str = who["H2Ps"];
                                if( do_check_Mj(str, LastDump)>1 ){
                                        //PAUSE = 1;
                                        do_pon( LastDump, ob );
        ...
        //for(i=0; i<3; i++) {
                if( !userp(ob) ) {
                        str = who["3Ps"];
                //ob = next_data(me->query("id"), i+1, "player");                       
                        if( do_check_Mj(str, LastDump)==3 && do_check_num(LastDump, 1)>27 && strlen(who["Mj"])<13){
                                //不立直就杠
                                //PAUSE = 1;
                                do_gon( LastDump, ob );
                        return 1;
        ...
        who = next_data(NowPlayerId[NowPlayer], 0, "data"); 
        str = "";
        if( who["Hu_type"]!="samecolor"||who["Hu_type"]!="twocolor" ) str = next_data(me->query("id"), 1, "2Es1");
        else str = next_data(me->query("id"), 1, "H2Es1");
        ...
        if( who["Hu_type"]!="samecolor"||who["Hu_type"]!="twocolor" ) str = next_data(me->query("id"), 1, "2Es2");
        else str = next_data(me->query("id"), 1, "H2Es2");
        ...
        if( userp(ob) ) {
                ...
                {
                        tell_object( ob, "你可以用"+HIR+"eat"+NOR+"来吃牌！！\n");
                        tell_object(ob, "以下是你的牌：\n" +show_mj( str, 6) );
                }
                        return 1;
        }
...
int dump_samecolor( string str_out, string str_mj, object me, mapping Temps )
{
        ...
        int size;
        //tempW = "";tempT = "";tempS = "";tempB = "";
        
        get_all_temps( Temps, str_out );
        //tell_object( present("wiman"), "out is "+str_out+" "+"tempw is "
        //      +Temps["tempW"]+"tempt is "+Temps["tempT"]+"temps is "+Temps["tempS"]);
        //tell_object( present("wiman"), "\nmj is "+str_mj);        
        if( Temps["tempB"]==""&&Temps["tempT"]==""&&Temps["tempS"]=="" ) {      
                size = strlen(Temps["tempW"]);
           get_all_temps( Temps, str_mj);
                //tell_object( present("wiman"), "tempw is "
                //      +Temps["tempW"]+"tempt is "+Temps["tempT"]+"temps is "+Temps["tempS"]);
        ...
        if( Temps["tempW"]==""&&Temps["tempB"]==""&&Temps["tempS"]=="" ) {
        ...
        if(Temps["tempW"]==""&&Temps["tempT"]==""&&Temps["tempB"]=="") {
        ...
        if(Temps["tempW"]==""&&Temps["tempT"]==""&&Temps["tempS"]=="") {
...
int get_all_temps( mapping map, string str )
{
        ...
        for(; size>=0; size-=2){
          ...
          else map["tempB"]+=str[size..size+1];
        }
}    
