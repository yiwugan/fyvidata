...
//      line = sprintf( BOLD "%s" NOR "%s\n\n", RANK_D->query_rank(ob), ob->short(1) );
        line = sprintf( "\n %s\n\n", ob->short(1) );
        line += sprintf(" 你是一%s"HIG"%s"NOR""HIR"%s"NOR"岁的%s%s，%s生。\n",
                ob->query("unit"),
        ...
        line += sprintf(" 你总共杀了"HIC"%s"NOR"个人，"RED"%s"NOR"个玩家，被杀了"WHT"%s"NOR"次。\n",
                chinese_number(my["MKS"] + my["PKS"]), chinese_number(my["PKS"]),chinese_number(my["KILLED"]));
        line += sprintf(" 你到目前为止总共完成了"CYN"%s"NOR"个使命，"HIG"%s"NOR"个组织任务。\n",
                chinese_number(my["TASK"]),chinese_number(my["QUESTNPC"]));
        line += sprintf(" 你到目前为止总共完成了"HIR"%s"NOR"点大理贡献。\n",
                chinese_number(ob->query("gongxian")));
        line += sprintf(" 你到目前为止总共完成了"HIY"%s"NOR"个特殊使命。\n",
                chinese_number(ob->query("bigtasks"))); 
        line += sprintf(" 你到目前为止总共完成了"HIG"%s"NOR"个十大恶人通缉任务。\n",
                        chinese_number(ob->query("xxxiang")));
      line += sprintf(" 你目前精力防御值为百分之"HIG"%s"NOR"。\n",
                              chinese_number(ob->query("resistance/gin"))); 
      line += sprintf(" 你目前气血防御值为百分之"HIR"%s"NOR"。\n",
                              chinese_number(ob->query("resistance/kee"))); 
      line += sprintf(" 你目前心神防御值为百分之"HIC"%s"NOR"。\n",
                              chinese_number(ob->query("resistance/sen"))); 
        line += sprintf(" 你总共战胜过"HIM"%s"NOR"个风云天骄。\n",
                chinese_number(sizeof(ob->query("m_killer"))));
        line += sprintf(" 你现在共有存款："HIY"%s"NOR"文钱。\n",
                chinese_number(ob->query("deposit")));
        line += sprintf(" 你到目前为止叛师记录是"RED"%s"NOR"次。\n",
