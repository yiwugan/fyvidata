...
int do_bet(string arg)
{
        ...
        if(!coin) return notify_fail("你身上没有钱。\n");
        if((int) coin->query_amount() < bet_amt)
        return notify_fail("你身上没有那麽多钱。\n"); 
        if((int) this_object()->query("lower_limit") > bet_amt)
        return notify_fail("这张台的最小赌注是"+chinese_number((int) this_object()->query("lower_limit"))+"两钱。\n");
   ...
   if((int) me->query("gamble/amount"))
       ...
       message_vision(sprintf("$N又从身上拿出%s文钱，加压在%s上．\n",
        ...
        if(bet_type >=3)
                message = sprintf(RED"%s"NOR,chinese_number(bet_type));
        message_vision(sprintf("$N从身上拿出%s文钱，压在%s上．\n",
                                chinese_number(bet_amt), message), me);
...
void rolldice()
{
        ...
        message_vision("$N大声叫道：下定注！说罢，将手中的叁粒骰子撒向碗里．\n",this_object());
        message_vision("\n．．．叁粒骰子在碗里蹦跳着，发出悦耳的声音．．．\n",this_object());
        pre_cal();
...
inv = all_inventory(environment(this_object()));
        ...
        if( userp(inv[i]) && (int) inv[i]->query("gamble/amount") )
        {
                ...
                {
                message_vision("赢家－$N，一赔叁十六！！\n",inv[i]);
                reward = (int) inv[i]->query("gamble/amount") * 36 ;
...
int accept_object(object who, object ob)
{
        ...
        if(bet_type >=3)
                message = sprintf(RED"%s"NOR,chinese_number(bet_type)); 
        tell_object(who,"黄痞子悄悄告诉你：下手儿压"+message+"，一定赢！\n");
        message_vision("$N会心的微笑起来．．\n",who);
        
                        return 1;
                } else {
                    say("黄痞子说道：这点钱你还是自己用吧！\n");
                        return 0;
                }
        return 0;
} 
